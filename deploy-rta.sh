...
display_banner() {
  local width=70
  local padding=$((width - 44)) # 44 is the length of the title
...
parse_arguments "$@"
main
exit 0    ["terminator"]="terminator --version | grep 'terminator'"
)
...
create_system_snapshot() {
  ...
  
  {
    ...
    $SCRIPTS_DIR/validate-tools.sh --essential-only | grep -E '^\[|\+|\-|\!]' | sed 's/^/  /'
    echo ""
    echo "=== DISK SPACE ==="
    df -h
    echo ""
...
install_main_installer() {
  ...
  cat > "$DEPLOY_DIR/rta_installer.sh" << 'EOL'
#!/bin/bash
# =================================================================
# Robust Kali Linux Security Tools Installer v2.0
##!/bin/bash
...
DESKTOP_ONLY=false
GITHUB_INSTALLER_URL="https://raw.githubusercontent.com/yourusername/rta-installer/main"
